...
interface XiaomiItem {
  name: string;
  price: number;
  image: string;
}

const xiaomiItems: XiaomiItem[] = [
  {
  ...
  {
    name: "Samsung Galaxy S21 Ultra 5G 12/128",
    price: 105990,
    image:
...
const XiaomiItems: React.FC = () => {
  return (
    <section className="XiaomiItems">
      <h1 className="text-3xl font-bold text-gray-900 ml-1 sm:mt-10 py-5">
        Продукт Xiaomi
      </h1>
      <div className="grid grid-cols-4 gap-5">
        {xiaomiItems.map((item, index) => (
          <figure
            key={index}
